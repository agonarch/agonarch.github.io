...
Note: key and reference are not valid, you need a valid customer key and reference. Don’t forget to URL encode it as required for all query parameters. See chapter 1.1 ‘Requesting for a token’ on how to obtain the required key. 
...
* Account = optional, the account where the shipment is booked on. If not specified the account belonging to the key is used. This can be used if the shipment is booked on a child account and if you want to access this shipment via Act on Behalf function with the key of the parent account. See chapter 'Act on behalf' below for more information.
...
Example:
You login as user of the ERPDEMO account, which is a main/parent account. A booking is then performed with this URL:

`POST request to /v2/shipments/*ERPDEMO*/BOOK`

This will book the shipment on account ERPDEMO. 

Now for example, account ERDEMOCHILD is a child account of ERPDEMO. If you want to book on account ERDEMOCHILD the flow is: +
1. You login as user of the ERPDEMO account (so the same user as above). +
2. A booking is then performed with this URL:

`POST request to /v2/shipments/*ERDEMOCHILD*/BOOK`
...
{
    "status": "BAD_REQUEST",
    "timestamp": "2024-02-28T12:42:49.450320273",
    "code": "TS222122",
